...
-- gen_random_uuid() is built into Postgres core as of v13, so no
-- extension is needed. This matters on managed Postgres (Supabase,
-- Cloud SQL) where the database role often isn't allowed to run
-- CREATE EXTENSION at all -- one less way for setup to fail.
