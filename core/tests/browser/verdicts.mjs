...
out.push(`verdicts are visually distinct (${colours.join(', ')})`);
...
out.push(`${links} source links, real urls, opened safely`);
...
out.push('claim text and unresolved notes carried through');
...
out.push('fits the landscape viewport exactly');
...
for (const [w, h, label] of [[820, 1180, 'iPad portrait'], [390, 844, 'iPhone']]) {
  ...
  let ok = true;
  if (r.scrollW > r.w + 1) { fail(`${label}: sideways scroll (${r.scrollW} > ${r.w})`); ok = false; }
  if (await p.locator('.verdict.contradicted').count() !== 1) { fail(`${label}: verdicts missing`); ok = false; }
  if (ok) out.push(`${label} (${w}×${h}): no sideways scroll, verdicts intact`);
